        ...
        @forelse($factories as $factory)
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col relative group">
                ...
                <div class="h-40 bg-slate-50 border-b border-slate-100 relative overflow-hidden">
                    @if($factory->logo)
                        <img src="{{ Storage::url($factory->logo) }}" alt="{{ $factory->name }}" class="w-full h-full object-cover opacity-90 group-hover:scale-105 transition-transform duration-500">
                    @else
                        ...
                        <div class="w-full h-full bg-gradient-to-br from-primary-50 to-primary-100 flex items-center justify-center">
                            <svg class="w-16 h-16 text-primary-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                        </div>
                    @endif

                    <!-- Country Badge -->
                    @if($factory->country)
                    <div class="absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest text-slate-700 shadow-sm border border-white flex items-center gap-2">
                        <span class="w-4 h-4 rounded-full overflow-hidden inline-block border border-slate-100">
                            ...
                            <img src="https://flagcdn.com/w40/{{ strtolower($factory->country->code ?? 'us') }}.png" alt="{{ $factory->country->name }}" class="w-full h-full object-cover">
                        </span>
                ...
                <div class="p-6 flex-1 flex flex-col">
                    ...
                    <div class="pt-4 border-t border-slate-100 mt-auto">
                        <a href="{{ route('factories.show', $factory) }}" class="flex items-center justify-between group/btn text-slate-900 hover:text-primary-600 transition-colors">
