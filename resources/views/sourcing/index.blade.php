...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="relative bg-primary-900 rounded-[3rem] overflow-hidden p-10 lg:p-20 mb-16 shadow-2xl">
        ...
        <div class="relative z-10 text-center max-w-3xl mx-auto">
            <h1 class="text-4xl lg:text-6xl font-bold text-white mb-6 leading-tight">Find Your <span class="text-primary-400 text-italic">Best Deal</span> Today</h1>
            <p class="text-white/80 text-lg lg:text-xl mb-10 leading-relaxed">Submit your request once, and let the world's leading optics manufacturers compete for your business.</p>
    ...
    <div class="flex flex-col lg:flex-row justify-between items-end mb-10 gap-4">
        <div>
            <h2 class="text-3xl font-bold text-slate-900">Active Requests</h2>
            <p class="text-slate-500 mt-2">Latest sourcing opportunities for manufacturers</p>
        </div>
    </div>
        ...
        <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col lg:flex-row lg:items-center gap-8 card-hover">
            <div class="flex-1">
                <div class="flex items-center gap-3 mb-4">
                    <span class="px-4 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold rounded-full uppercase tracking-widest">{{ $request->category->name['en'] }}</span>
            ...
            <div class="flex items-center gap-8 lg:text-right shrink-0">
                <div>
                    <p class="text-[10px] text-slate-400 uppercase font-bold tracking-widest mb-1">Quantity</p>
                    <p class="text-xl font-bold text-slate-900">{{ $request->quantity }}</p>
                </div>
                <div>
                    <p class="text-[10px] text-slate-400 uppercase font-bold tracking-widest mb-1">Target Price</p>
                    <p class="text-xl font-bold text-primary-600">{{ $request->currency }} {{ number_format($request->target_price, 0) }}</p>
                </div>
                <a href="{{ route('sourcing.show', $request) }}" class="px-8 py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all">View Details</a>
            </div>
        </div>
        @empty
